                    ...
                    <div className="space-y-3">
                        {formData.items.map((item, index) => {
                            ...
                            return <div key={index} className="grid grid-cols-1 sm:grid-cols-5 gap-3 items-center bg-amber-50 p-3 rounded-lg border border-amber-100">
                                ...
                                <input type="number" min="1" max={stocks.find(s => s.id === Number(item.productId))?.remaining || 99} placeholder={`Max: ${stocks.find(s => s.id === Number(item.productId))?.remaining || ''}`} value={item.quantity} onChange={(event) => { const max = stocks.find(s => s.id === Number(item.productId))?.remaining || 99; const val = Math.min(Number(event.target.value), max); updateItem(index, { quantity: val > 0 ? val : event.target.value }); }} className="border-gray-300 rounded-lg border p-2.5 focus:ring-2 focus:ring-amber-500" />
